...
class TriplePanna extends StatefulWidget {
  String openingTime;
  String closingTime;
  String cardId;
  ...
  @override
  _TriplePanna createState() => _TriplePanna();
}

class _TriplePanna extends State<TriplePanna> {
  ...
  Future<void> _onSubmit() async {
    ...
    String digit =
    controller.tripplePannaDigitController.text.trim();

    String amount =
    controller.tripplePannaAmountController.text.trim();
    double? checkBidAmount = double.tryParse(controller.tripplePannaAmountController.text);


    // Check if the entered digit matches any item in the suggestion list
    bool isValidDigit = gameScreensController.triplePannaSuggestionList.contains(digit);
    ...
    if (!isValidDigit) {
      ...
      await ShowAlertMessage.showSimpleDialog(
        context,
        title: "Invalid Digit",
        message: "The entered triple panna digit is not valid.",
        isSuccess: false,
      );
    ...
    if (isCloseResultActive) {
      ...
      if (!validationResult[0]) {
        ...
        double bidAmount = double.tryParse(controller.tripplePannaAmountController.text.trim()) ?? 0.0;
        double digit = double.tryParse(controller.tripplePannaDigitController.text.trim()) ?? 0.0;
  ...
  void _confirmBid({required amount})  async {
   try{
     ...
     final applyBid = PaymentPayload(
       ...
       amount: controller.tripplePannaAmountController.text,
       digit: controller.tripplePannaDigitController.text,
     ...
     Customer response =
     await ApiDataSourceImpl().bidPlace(authToken: authToken, bidDetails: applyBid);
     controller.currentBalance.value = (double.tryParse(response.data[0].accountBalance.toString()) ?? 0.0);

     controller.tripplePannaDigitController.clear();
     controller.tripplePannaAmountController.clear();
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                CustomInputBox(
                  title: 'Enter Tripple Paana',
                  isInputEnabled: true,
                  controller: controller.tripplePannaDigitController,
                  maxInputLength: 3,
                  keyboardType: TextInputType.number,
                  suggestions: gameScreensController.triplePannaSuggestionList
                ),
                const SizedBox(
                  height: 15,
                ),
                InputBox(
                  ...
                  isInputEnabled: true,
                  title: 'Enter Amount',
                  controller: controller.tripplePannaAmountController,
                ),
                const SizedBox(
                  height: 15,
                ),
