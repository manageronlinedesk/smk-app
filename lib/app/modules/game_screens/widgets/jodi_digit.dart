...
class JodiDigit extends StatefulWidget {
  String openingTime;
  String closingTime;
  String cardId;
  ...
  @override
  _JodiDigit createState() => _JodiDigit();
}

class _JodiDigit extends State<JodiDigit> {
  ...
  Future<void> _onSubmit() async {
    ...
    String digit = controller.jodiDigitController.text.trim();

    String amount =
    controller.jodiAmountController.text.trim();
    double? checkBidAmount = double.tryParse(controller.jodiAmountController.text);



    // Check if the entered digit matches any item in the suggestion list
    bool isValidDigit = gameScreensController.jodiDigitSuggestionList.contains(digit);
    ...
    if (!isValidDigit) {
      ...
      await ShowAlertMessage.showSimpleDialog(
        context,
        title: "Invalid Digit",
        message: "The entered jodi digit is not valid.",
        isSuccess: false,
      );
    ...
    if (isCloseResultActive) {
      ...
      if (!validationResult[0]) {
        ...
        double bidAmount = double.tryParse(controller.jodiAmountController.text.trim()) ?? 0.0;
        double digit = double.tryParse(controller.jodiDigitController.text.trim()) ?? 0.0;
  ...
  void _confirmBid({required amount})  async{
   try{
     ...
     final applyBid = PaymentPayload(
       ...
       amount: controller.jodiAmountController.text,
       digit: controller.jodiDigitController.text,
       isOpen: isOpenSelected.toString(),
       adminId: AppValues.adminId,
     );
     String? authToken = await controller.getAuthToken();
     Customer response = await controller.apiDataSource.bidPlace(authToken: authToken, bidDetails: applyBid);
     controller.currentBalance.value = (double.tryParse(response.data[0].accountBalance.toString()) ?? 0.0);

     controller.jodiAmountController.clear();
     controller.jodiDigitController.clear();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title:
          const Text('Jodi Digit', style: TextStyle(color: AppColors.colorWhite)),
        ...
        body: Container(
          ...
          child: Column(
            children: [
              ...
              CustomInputBox(
                title: 'Enter Jodi Digit',
                isInputEnabled: true,
                controller: controller.jodiDigitController,
                maxInputLength: 2,
                keyboardType: TextInputType.number,
                suggestions: gameScreensController.jodiDigitSuggestionList,
              ),
              ...
              InputBox(
                ...
                isInputEnabled: true,
                title: 'Enter Amount',
                controller: controller.jodiAmountController,
              ),
              const SizedBox(
                height: 15,
              ),
