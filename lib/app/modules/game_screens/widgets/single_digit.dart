...
class SingleDigit extends StatefulWidget {
  ...
  @override
  _SingleDigit createState() => _SingleDigit();
}

class _SingleDigit extends State<SingleDigit> {
  ...
  void initState() {
    ...
    DateTime openingTime = DateFormat('dd-MM-yyyy hh:mm a').parse('${dt + widget.openingTime!}');
    DateTime closingTime = DateFormat('dd-MM-yyyy hh:mm a').parse('${dt + widget.closingTime!}');

    _timer = Timer.periodic(Duration(milliseconds: initialSeconds), (Timer timer) {
  ...
  Future<void> _onSubmit() async {
    ...
    String digit =
    controller.singleDigitController.text.trim();

    String amount =
    controller.singleAmountController.text.trim();

    double? checkBidAmount = double.tryParse(controller.singleAmountController.text);


    bool isValidDigit = gameScreensController.singleDigitSuggestionList
        .contains(digit);
    ...
    if (!isValidDigit) {
      ...
      await ShowAlertMessage.showSimpleDialog(
        context,
        title: "Invalid Digit",
        message: "The entered single digit is not valid.",
        isSuccess: false,
      );
    ...
    if (isCloseResultActive) {
      ...
      List validationResult = InputValidation.validator(
          Digit: digit,
          amount: amount,
          currentBalance: controller.currentBalance.value.toString());
      if (!validationResult[0]) {
        ...
        double bidAmount = double.tryParse(controller.singleAmountController.text.trim()) ?? 0.0;
        double digit = double.tryParse(controller.singleDigitController.text.trim()) ?? 0.0;
  ...
  void _confirmBid({required amount}) async {
   try{
     ...
     final applyBid = PaymentPayload(
       ...
       amount: controller.singleAmountController.text,
       digit: controller.singleDigitController.text,
       isOpen: isOpenSelected.toString(),
       adminId: AppValues.adminId,
     );
     String? authToken = await controller.getAuthToken();
     Customer response =
     await controller.apiDataSource.bidPlace(
         authToken: authToken, bidDetails: applyBid);
     controller.currentBalance.value = (double.tryParse(response.data[0].accountBalance.toString()) ?? 0.0);

     controller.singleDigitController.clear();
     controller.singleAmountController.clear();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: const Text('Single Digit',
              style: TextStyle(color: AppColors.colorWhite)),
          iconTheme: const IconThemeData(color: AppColors.colorWhite), // Set the icon color here
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                CustomInputBox(
                  title: 'Enter Single Digit',
                  isInputEnabled: true,
                  controller: controller.singleDigitController,
                  maxInputLength: 1,
                  keyboardType: TextInputType.number,
                  suggestions: gameScreensController.singleDigitSuggestionList,
                ),
                const SizedBox(
                  height: 15,
                ),
                InputBox(
                  ...
                  isInputEnabled: true,
                  title: 'Enter Amount',
                  controller: controller.singleAmountController,
                ),
                const SizedBox(
                  height: 15,
                ),
