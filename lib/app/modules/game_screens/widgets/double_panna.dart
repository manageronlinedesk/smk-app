...
class DoublePanna extends StatefulWidget {
  ...
  @override
  _DoublePanna createState() => _DoublePanna();
}

class _DoublePanna extends State<DoublePanna> {
  ...
  Future<void> _onSubmit() async {
    ...
    String digit = controller.doublePannaDigitController.text.trim();
    String amount = controller.doublePannaAmountController.text.trim();
    double? checkBidAmount = double.tryParse(controller.doublePannaAmountController.text);


    // Check if the entered digit matches any item in the suggestion list
    bool isValidDigit = gameScreensController.doublePannaSuggestionList.contains(digit);

    if (!isValidDigit) {
      await ShowAlertMessage.showSimpleDialog(
        context,
        title: "Invalid Digit",
        message: "The entered double panna digit is not valid.",
        isSuccess: false,
      );
    ...
    if (isCloseResultActive) {
      String selectedOption = isOpenSelected ? 'Open' : 'Close';

      List validationResult = InputValidation.validator(Digit: digit, amount: amount, currentBalance: controller.currentBalance.value.toString());
      ...
      if (!validationResult[0]) {
        ...
        double bidAmount = double.tryParse(controller.doublePannaAmountController.text.trim()) ?? 0.0;
        double digit = double.tryParse(controller.doublePannaDigitController.text.trim()) ?? 0.0;
  ...
  void _confirmBid({required amount}) async{
    try{
      ...
      final applyBid = PaymentPayload(
        ...
        amount: controller.doublePannaAmountController.text,
        digit: controller.doublePannaDigitController.text,
        isOpen: isOpenSelected.toString(),
        adminId: AppValues.adminId,
      );
      String? authToken = await controller.getAuthToken();
      Customer response = await controller.apiDataSource
          .bidPlace(authToken: authToken, bidDetails: applyBid);
      controller.currentBalance.value = (double.tryParse(response.data[0].accountBalance.toString()) ?? 0.0);

      controller.doublePannaAmountController.clear();
      controller.doublePannaDigitController.clear();
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                CustomInputBox(
                  title: 'Enter Double Paana',
                  isInputEnabled: true,
                  controller: controller.doublePannaDigitController,
                  maxInputLength: 3,
                  keyboardType: TextInputType.number,
                  suggestions: gameScreensController.doublePannaSuggestionList,
                ),
                const SizedBox(
                  height: 15,
                ),
                InputBox(
                  ...
                  isInputEnabled: true,
                  title: 'Enter Amount',
                  controller: controller.doublePannaAmountController,
                ),
                const SizedBox(
                  height: 15,
                ),
